...
## 套件管理命令
- 所有 Python 相依套件**必須**使用 uv 來安裝、同步和鎖定
- 絕對不要直接使用 pip、pip-tools、poetry 或 conda 進行相依套件管理
...
- 執行 Python 工具如 Pytest：`uv run pytest` 或 `uv run ruff`
...
========================================
[✓] 當前使用：API Key 模式
[i] API Key 前綴：sk-ant-api03-xxxxx...
========================================
```
...
**判斷方式：**
- 如果**沒有輸出**或輸出為**空白** → 使用訂閱身分模式
- 如果**顯示 API Key** → 使用 API Key 模式
...
### 日常工作流程
1. **預設使用訂閱身分**：日常開發時使用訂閱額度
2. **遇到限流時切換**：當看到速率限制訊息時，執行 `docs\set_claude_use_mode\use-api.bat`
3. **完成後切回**：使用完 API 額度後，執行 `docs\set_claude_use_mode\use-subscription.bat` 切回訂閱身分
...
# 2. 切換模式（如果需要）
  在終端機輸入 `docs\set_claude_use_mode\use-api.bat` 或 `docs\set_claude_use_mode\use-subscription.bat`

# 3. 在同一個終端機中使用 Claude Code
# 或使用 VS Code 的 Claude Code 擴充功能
```
...
A: 環境變數只在當前終端機視窗有效。請確保：
   1. 在執行腳本的**同一個終端機視窗**中使用 Claude Code
