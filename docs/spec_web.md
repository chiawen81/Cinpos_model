...
> **版本**: 1.1
> **更新日期**: 2025-11-20
> **簡介**: 本文件記錄電影票房預測系統的所有頁面開發需求、互動邏輯和業務規則
...
電影票房預測網站是一個為電影發行商和戲院提供票房預測和分析的工具，協助決策者：
- 📊 **即時監控**: 追蹤電影票房表現和趨勢
- 🎯 **預測分析**: 預測未來週次的票房走勢
- ⚠️ **預警提示**: 識別票房異常衰退的電影
- 📈 **數據視覺化**: 透過圖表呈現票房趨勢和分析結果

### 主要功能

1. **首頁儀表板**: 快速概覽近期上映電影、追蹤清單、預警資訊
2. **電影詳細頁**: 單部電影的完整票房歷史、預測和分析
3. **預測功能**: 支援已上映和新電影的票房預測
4. **追蹤管理**: 追蹤關注的電影並接收預警通知
5. **衰退預警**: 自動偵測票房異常衰退並分級提示

### UI 設計風格

- **主色調**: 紫藍漸層 (#9C6DFF → #71DDFF)
- **背景色**: 深色系 (#1a1a1a, #2a2a2a)
- **強調色**: 淺紫色 (#DADAFF)
- **設計元素**: 圓角、漸層按鈕、玻璃質感卡片、現代科技感
...
│  │  - stats_service.py         │   │
│  │  - boxoffice_list_service.py│   │
│  │  - decline_warning_service.py│  │
│  └─────────────────────────────┘   │
│             │                       │
│  ┌─────────────────────────────┐   │
│  │  Utils (工具層)             │   │
...
│   ├── stats_api.py          # 統計 API 路由
│   └── boxoffice_list_api.py # 票房列表 API 路由
├── services/                 # 業務邏輯層
│   ├── movie_service.py
│   ├── prediction_service.py
│   ├── stats_service.py
...
此圖表整合歷史票房數據與未來預測，協助使用者快速掌握票房走勢與預測結果。

**圖表元素說明**：

| 元素 | 顏色/樣式 | 說明 |
|-----|---------|------|
| **實際票房** | 🔵 藍色實線 (`#71DDFF`) | 已結束週次的實際票房數據<br>- 每週一個圓點標記<br>- 資料來源：歷史票房記錄 |
| **預測票房** | 🟣 紫色虛線 (`#9C6DFF`) | 未來週次的預測票房<br>- 從歷史最後一點延伸<br>- 每週一個圓點標記<br>- 虛線樣式表示預測性質 |
| **信心區間** | 🟣 紫色陰影 (透明度 20%) | 預測的可能誤差範圍<br>- 包圍預測線的陰影區域<br>- 表示預測值的不確定性<br>- 約有 95% 機率真實票房落在此範圍內 |
...
3. **信心區間評估**：
   - **陰影越窄** → 模型越有把握，預測越可靠
   - **陰影越寬** → 不確定性越高，預測誤差可能較大
   - 真實票房落在陰影範圍內的機率約為 95%
   - 若實際票房超出信心區間，表示發生了模型未預期的狀況

**範例解讀**：
```
情境：某電影歷史票房從第1週的150萬逐漸衰退至第3週的160萬
預測：第4週預測票房120萬，信心區間為100萬~140萬

解讀：
- 票房呈下降趨勢，預測將持續衰退
- 第4週票房最可能落在100萬到140萬之間
- 信心區間寬度40萬（140-100），表示預測有一定不確定性
- 若實際票房低於100萬或高於140萬，表示出現異常情況
```

**實作細節**：
- **檔案**：`src/web/business/detail/static/js/common/charts.js:73-176`
- **函數**：`drawBoxOfficeTrendChart(container, data)`
- **連接點處理**：預測線與歷史線在最後一個歷史點自然連接，確保視覺連續性
- **資料欄位**：
  - 歷史資料使用 `boxoffice` 欄位
  - 預測資料使用 `predicted_boxoffice` 欄位
  - 信心區間使用 `confidence_lower` 和 `confidence_upper` 欄位
...
週衰退率分析圖表使用顏色區分衰退程度，幫助快速識別票房表現：

| 顏色 | 衰退率範圍 | 十六進位色碼 | 說明 |
|------|-----------|------------|------|
| **🟢 綠色** | ≥ -10% | `#51CF66` | 衰退正常或成長，票房表現穩定 |
| **🟡 黃色** | -30% ~ -10% | `#FFDD00` | 輕微衰退，在正常範圍內 |
| **🟠 橘色** | -50% ~ -30% | `#FFA500` | 中度衰退（警戒），需要關注 |
| **🔴 紅色** | < -50% | `#FF4444` | 嚴重衰退（危險），異常衰退 |
...
**實作位置**：
- **檔案**：`src/web/business/detail/static/js/common/charts.js:235-240`
- **函數**：`drawDeclineRateChart(container, data)`
- **邏輯**：根據衰退率動態設定柱狀圖顏色

```javascript
marker: {
    color: declineRates.map(r => {
        if (r < -0.5) return '#FF4444';      // 紅色：衰退 > 50%
        if (r < -0.3) return '#FFA500';      // 橘色：衰退 30-50%
        if (r < -0.1) return '#FFDD00';      // 黃色：衰退 10-30%
        return '#51CF66';                     // 綠色：衰退 < 10% 或成長
    })
}
```
...
### 專案通用
- [專案說明（CLAUDE.md）](../../CLAUDE.md) - 專案的整體說明和開發指引
