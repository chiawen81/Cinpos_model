...
def clean_omdb_json_to_csv():
    ensure_dir(OMDB_PROCESSED)
    rows = []

    for file in os.listdir(OMDB_RAW):
        if not file.endswith(".json"):
            continue
        path = os.path.join(OMDB_RAW, file)
    ...
    output_path = os.path.join(OMDB_PROCESSED, f"movieInfo_omdb_{today}.csv")
